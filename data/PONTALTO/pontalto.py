...
class PONTALTO(Dataset):
    def __init__(self, db_path, split, transforms, nameset='train', img_size=[512, 32], charset=None):
        ...
        # Add numbers and Italian alphabet (uppercase and lowercase) if not in charset
        italian_chars = "àèéìíîòóùú"
        additional_chars = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz" + italian_chars + italian_chars.upper()
        if charset is not None:
            # Ensure all required characters are in the charset
    ...
    def __getitem__(self, index):
        ...
        img_name, text = sample['img'], sample['text']
...
if __name__ == '__main__':
    pontalto_path = './data/PONTALTO'

    # LAM(lam_path, 'leave_decade_out/leave_decade_1_out', ToTensor(), nameset='train')
    train_dataset = PONTALTO(pontalto_path, 'basic', ToTensor(), nameset='train')
    test_dataset = PONTALTO(pontalto_path, 'basic', ToTensor(), nameset='test', charset=train_dataset.charset)
    ...
    # for idx, (img, texts) in enumerate(train_loader):
    #     # do stuff
    #     print('train', idx, len(train_loader), texts)
    #     exit()

    for idx, (img, texts) in enumerate(test_loader):
        # do stuff
