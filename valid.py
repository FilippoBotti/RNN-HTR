...
def validation(model, criterion, evaluation_loader, converter):
    ...
    for i, (image_tensors, labels) in tqdm(enumerate(evaluation_loader)):
        ...
        #     img = image_tensors[j].cpu().numpy().squeeze()
        #     plt.figure(figsize=(8, 4))
        #     plt.imshow(img, cmap='gray')
        #     plt.text(50, 140, f"Label: \n{labels[j]}\n\nPrediction:\n{preds_str[j]}", size='large')
        #     plt.axis('off')
        #     plt.tight_layout()
        #     plt.savefig(f'./results/b{i}_{j}.png')
        #     plt.close()
