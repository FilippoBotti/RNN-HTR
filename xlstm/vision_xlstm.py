...
class ViLBlock(nn.Module):
    def __init__(self, dim, direction, drop_path=0.0, norm_bias=False):
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim * 4.), act_layer=nn.GELU, drop=0.0)
    ...
    def forward(self, x):
        B, L, C = x.shape
        x = x*self.skip_scale + self.drop_path(self.layer(self.norm(x)))
        x = x*self.skip_scale2 + self.drop_path2(self.mlp(self.norm2(x)))
        return x
...
if __name__ == "__main__":
    ...
    B, S, H = 2, 16, 32
    x = torch.randn(B, S, H)
    layer = ViLBlock(dim=H, direction=SequenceTraversal.ROWWISE_FROM_TOP_LEFT)
    out = layer(x)
    print(out.shape)  # should be (B, S, H)
