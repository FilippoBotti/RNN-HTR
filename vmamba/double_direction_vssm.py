...
class SS2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, direction=1, **kwargs):
        B, H, W, C = x.shape
...
class VSSBlockDouble(nn.Module):
    def __init__(
            self,
            hidden_dim: int = 0,
            drop_path: float = 0,
        ...
        self.self_attention = SS2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, args=args, **kwargs)
