...
class SS2Dv2:
    def __initv2__(
        ...
        **kwargs,    
        ...
        # for _ in range(self.k_group):
        #     self.combs.append(nn.Conv2d(in_channels=self.d_inner, out_channels=9, kernel_size=3, padding=1))
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            # xs = xs.reshape([K,B,D,H,W])
            # weights = torch.stack([self.combs[d](xs[d]) for d in range(K)], dim=0)  # (K, B, 9, H, W)
            # weights = torch.nn.functional.softmax(weights, dim=2)

            # # # Applichiamo Unfold in parallelo
            # xs = xs.view(K * B, D, H, W)  # Unfold richiede input (B, C, H, W)
            # output = self.unfold(xs).view(K, B, D, 3 * 3, H, W)  # (K, B, D, 9, H, W)
            # rimuovi_qui
            # # Usare torch.einsum per evitare operazioni in-place
            # xs = torch.einsum("kbdfhw, kbfhw -> kbdhw", output, weights)
            # xs = xs.reshape([B,K,D,H*W]).contiguous()
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        ssm_init="v0",
        forward_type="v2",
        scan_mode="bidi",  # Add scan_mode parameter
        # =============================
        mlp_ratio=4.0,
        ...
        **kwargs,
        ...
        if self.ssm_branch:
            ...
            self.op = _SS2D(
                ...
                scan_mode=scan_mode,  # Pass scan_mode to SS2D
            )
        ...
        # self.first_attn = SelfAttention(hidden_dim)
        # self.first_mlp = Mlp(in_features=hidden_dim, hidden_features=hidden_dim*2, out_features=hidden_dim)
        # self.second_attn = SelfAttention(hidden_dim)
        # self.second_mlp = Mlp(in_features=hidden_dim, hidden_features=hidden_dim*2, out_features=hidden_dim)
        # self.SS = SoftSort()
        # self.permutation_weights = nn.Linear(in_features=hidden_dim, out_features=1)
        # self.phi = nn.Parameter(torch.randn(1,hidden_dim, num_patches*num_patches))
        self.kernel_size = depth
        self.permutation_weights = nn.Conv2d(in_channels=hidden_dim, out_channels=self.kernel_size*self.kernel_size, kernel_size=self.kernel_size, padding=int((self.kernel_size-1)/2))
        self.unfold = torch.nn.Unfold(kernel_size=(self.kernel_size,self.kernel_size), padding=int((self.kernel_size-1)/2))
...
class VSSM(nn.Module):
        ...
        **kwargs,
        ...
        for i_layer in range(self.num_layers):
            ...
            self.layers.append(self._make_layer(
                ...
                layer_depth=kernel_size[i_layer]
            ))
        # exit()
        ...
        # Attention
        # self.first_attn = SelfAttention(dims[0])
        # self.first_mlp = Mlp(in_features=dims[0], hidden_features=dims[1], out_features=dims[0])
        # self.second_attn = SelfAttention(dims[0])
        # self.second_mlp = Mlp(in_features=dims[0], hidden_features=dims[1], out_features=dims[0])

        #linear comb
        # self.permutation_weights = nn.Conv2d(in_channels=dims[0], out_channels=3, kernel_size=3, padding=1)
        
        #affinity matrix
        # self.permutation_weights = nn.Conv1d(in_channels=dims[0], out_channels=3136, kernel_size=1)
        
        #phi
        # self.phi = nn.Parameter(torch.randn(1,dims[0], 3136))
    ...
    def forward(self, input: torch.Tensor):
        ...
        if self.pos_embed is not None:
            ...
            x = x + pos_embed
        # Fixed random order for patches
        # x = x[:,:,self.indexes[0],:]
        # x = x[:,:,:,self.indexes[1]]
        ...
        # affinity_map = self.permutation_weights(x)  # Forma: [B, HW, HW] (affinità tra patch)

        # # 2. Calcolare la matrice di affinità tra patch con prodotto scalare (esempio)
        # affinity_matrix = torch.matmul(affinity_map.permute(0, 2, 1), affinity_map)  # Forma: [B, HW, HW]

        # # 3. Applicare softmax lungo l'asse 2 per ottenere P_hat (probabilità di ordinamento)
        # P_hat = torch.softmax(affinity_matrix, dim=2)
        # x = torch.bmm(P_hat, x.permute(0,2,1))
        # x = torch.reshape(x, [B,C,H,W])
        
        # linear comb
        # x = torch.reshape(x, [B,C,H,W])
        # weight = self.permutation_weights(x)
        # weight = torch.nn.functional.softmax(weight, dim=1)
        # # Usare Unfold per srotolare l'immagine
        # unfold = torch.nn.Unfold(kernel_size=(5,5), padding=2)
        # output = unfold(x).view(B,C,5*5,H,W)
        # # Moltiplicazione delle patch con il filtro normalizzato
        # x = einops.einsum(output, weight, "b c f h w, b f h w -> b c h w")
        

        # Attention
        # x = torch.reshape(x, [B,C,H*W])
        # x = x.permute(0,2,1)
        # perm = self.first_mlp(self.first_attn(x))
        # perm = self.second_mlp(self.second_attn(perm))
        # perm = self.permutation_weights(perm).permute(0,2,1).squeeze(1)
        # if self.training:
        #     P_hat = self.SS(perm).contiguous() 
        #     x = torch.bmm(P_hat, x)
        # else:
        #     indexes = torch.argsort(perm, descending=True)
        #     x = torch.stack([x[b, indexes[b]] for b in range(B)])
        #     P_hat = self.SS(perm).contiguous() 
        #     print(torch.mean(x - torch.bmm(P_hat, x)))
        # x = x.permute(0, 2, 1) 
        # x = torch.reshape(x, [B,C,H,W])
        ...
        # x = torch.reshape(x, [B,C,H*W])
        # x = x.permute(0,2,1)
        # d = torch.softmax(x@self.phi, dim=1)
        # x = d.permute(0,2,1) @ x
        # x = x.permute(0, 2, 1) 
        # x = torch.reshape(x, [B,C,H,W])
        
        # conv2d
        # perm = self.permutation_weights(x).reshape([B,1,H*W]).squeeze(1)
        # P_hat = self.SS(perm).contiguous() 
        # x = torch.reshape(x, [B,C,H*W])
        # x = torch.bmm(P_hat, x.permute(0,2,1))
        # x = torch.reshape(x, [B,C,H,W])
