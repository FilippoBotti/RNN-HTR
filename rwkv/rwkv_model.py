...
wkv6_cuda = load(name="wkv6",
                 sources=["./rwkv/cuda/wkv6_op.cpp", 
                          "./rwkv/cuda/wkv6_cuda.cu"],
...
class VRWKV_SpatialMix_V6(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, layer_id, patch_resolution, shift_mode='q_shift_multihead',
    ...
    def forward(self, x):
        def _inner_forward(x):
            ...
            r, k, v, g, w = self.jit_func(x, self.patch_resolution)
...
class VRWKV_ChannelMix(nn.Module):
    ...
    def forward(self, x):
        def _inner_forward(x):
            xx = self.shift_func(x, self.shift_pixel, patch_resolution=self.patch_resolution,
...
class RWKV_Block(nn.Module):
    def __init__(self, n_embd, n_head, n_layer, layer_id, patch_resolution=None, shift_mode='q_shift_multihead',
        ...
        self.att = VRWKV_SpatialMix_V6(n_embd, n_head, n_layer, layer_id, patch_resolution, shift_mode,
                                       shift_pixel, init_mode, key_norm=key_norm,
                                       with_cls_token=with_cls_token)

        self.ffn = VRWKV_ChannelMix(n_embd, n_head, n_layer, layer_id, patch_resolution, shift_mode,
...
if __name__ == '__main__':
    model = RWKV_Block(n_embd=768, n_head=8, n_layer=12, layer_id=0, patch_resolution=(16, 16), shift_mode='q_shift_multihead', shift_pixel=1,
                          drop_path=0.1, hidden_rate=4, init_mode='fancy', init_values=0.1,
                          post_norm=False, key_norm=False, with_cls_token=False, with_cp=False).cuda()
